    ...
    half3 ExponentialHeightFog(half3 col, half3 posWorld)
    {
        ...
        half falloff = heightFallOff * ( posWorld.y -  (_WorldSpaceCameraPos.y + _FogHeight)); //高度
        ...
        half distanceFactor = max((rayLength - _FogStartDis)/ 100.0, 0);
        half fog = fogFactor * fogDensity * distanceFactor;//插值

        //WorldSpaceLightDir(half4(posWorld,1))
        half inscatterFactor = pow(saturate(dot(-normalize(viewDir), _WorldSpaceLightPos0)), _FogInscatteringExp);
